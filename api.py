...
async def lifespan(app: FastAPI):
    git_persist.pull_latest(ROOT)
    if JSON_OUT.exists():
        try:
            from scripts.build_dashboard import build
            build(JSON_OUT, TEMPLATE_PATH, HTML_OUT)
        except Exception as exc:
            print(f"[startup] dashboard rebuild skipped: {exc}")
    yield


app = FastAPI(title="ALT Mobility - Insurance PDF Ingestion", lifespan=lifespan)
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        git_persist.commit_and_push(ROOT, file.filename)
